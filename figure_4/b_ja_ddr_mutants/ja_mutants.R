...
# Read data
data <- read.csv("12_ja_mutants/ja_mutants.csv")
...
# Two way anova
anova <- aov(ja_response ~ treatment*genotype, data = data)
summary(anova)
export::table2csv(anova, "12_ja_mutants/two_way_anova_result.csv")


capture_anova <- summary(anova)
capture.output(capture_anova, file = "12_ja_mutants/anova_results.doc")
...
write.csv(tukey_capture, "12_ja_mutants/tukey.csv")
...
# Plot 
ggplot(data, aes(x = treatment, y = ja_response)) +
  # Geoms
  stat_summary(
    aes(color = treatment),
    geom = "crossbar",
  ...
  # Labels
  labs(
    #title = "JA induction of DDR mutants",
    x = "DNA source",
...
ggsave("12_ja_mutants/ja_mutants.pdf", width = 9, height = 4.5, units = "cm", scale = 1, dpi = 300, device = cairo_pdf)

save.image(file = "12_ja_mutants/ja_mutants.RData")